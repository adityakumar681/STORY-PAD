...
const Profile = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setIsFollowing(response.data.followers?.some(follower => follower._id === currentUser?.id) || false);
    } catch (error) {
    ...
    try {
      ...
      const storiesWithChapters = await Promise.all(
        userStories.map(async (story) => {
          try {
            const chaptersResponse = await api.get(`/chapters/story/${story._id}`);
            return {
              ...story,
            ...
            {stories.length > 0 ? (
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-500">
                        <div className="flex items-center space-x-3">
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="font-medium">{story.likes?.length || 0}</span>
                          </div>
                          <div className="flex items-center space-x-1">
                            <BookOpen className="h-3 w-3 text-orange-400" />
                            <span className="font-medium">{story.chapterCount || 0}ch</span>
                          </div>
                        </div>
              ...
              <div className="text-center py-12 sm:py-16 lg:py-24 bg-white rounded-2xl border border-gray-100 shadow-sm">
                ...
                <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3 sm:mb-4">
                  {isOwnProfile ? 'Start Your Writing Journey' : 'No Stories Yet'}
                </h3>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-center justify-center min-h-[80vh] px-4">
          <div className="text-center max-w-md mx-auto">
            ...
            <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">User Not Found</h3>
            <p className="text-gray-600 mb-6 text-sm sm:text-base">The profile you're looking for doesn't exist or may have been removed.</p>
            <button
              ...
              <div className="flex-1 min-w-0 text-center sm:text-left">
                ...
                <div className="flex items-center justify-center sm:justify-start gap-8 sm:gap-10 mb-6">
                  <div className="text-center group">
                    <div className="text-xl sm:text-2xl font-bold text-gray-900">{getUserStats().stories}</div>
                  ...
                  >
                    <div className="text-xl sm:text-2xl font-bold text-gray-900 group-hover:text-orange-600 transition-colors">{getUserStats().followers}</div>
                  ...
                  <button 
                    ...
                    className="text-center hover:bg-gray-50 rounded-lg px-3 py-2 transition-all duration-200 hover:scale-105 group"
                  >
                    <div className="text-xl sm:text-2xl font-bold text-gray-900 group-hover:text-orange-600 transition-colors">{getUserStats().following}</div>
          ...
          <div className="relative bg-white rounded-2xl max-w-md w-full max-h-96 overflow-hidden shadow-xl border border-gray-200 transform transition-all duration-300 scale-100">
            ...
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg sm:text-xl font-bold text-gray-900">{userModal.title}</h3>
              </div>
              <button
                onClick={() => setUserModal({ show: false, users: [], title: '' })}
            ...
            <div className="overflow-y-auto max-h-80">
              {userModal.users && userModal.users.length > 0 ? (
                ...
                <div className="p-8 sm:p-12 text-center">
                  ...
                  <h4 className="font-semibold text-gray-900 mb-2 text-sm sm:text-base">No {userModal.title.toLowerCase()} yet</h4>
                  <p className="text-gray-500 text-xs sm:text-sm">
                    {userModal.title === 'Followers' ? 'Start sharing amazing stories to gain followers!' : 'Follow other storytellers to see their content!'}
                  </p>
                </div>
              )}
            </div>
